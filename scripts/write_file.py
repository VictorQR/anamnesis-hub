#!/usr/bin/env python3
"""
跨平台文本文件写入脚本 (cross-platform-writer)
==============================================

适配自 QClaw text-file-writer，用于 Victor 的 OpenClaw 环境。

解决 OpenClaw write 工具硬编码 utf-8 (无 BOM) 导致 Windows Excel
打开 CSV/TSV 文件时中文乱码的问题。

脚本路径: ~/.openclaw/workspace/user_workspace/skills/cross-platform-writer/scripts/write_file.py
