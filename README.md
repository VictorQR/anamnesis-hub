...
| **L2** 长期记忆 | 持久化事实、用户画像、永久决策 | `MEMORY.md`（只读基底 + auto-memory 自动写入） |

**自动化管线**：Dreaming（每日 UTC 03:00 自动分析会话 → 提炼 → 提升至 L2）、三方同步（Cloud ↔ 文件 ↔ 向量库）、auto-memory v2（读取 MemOS 云端已提取事实 → qwen3 去重格式化 → MEMORY.md）、Wiki 编译（实体提取 → 知识库）。

### 附赠技能
- **cross-platform-writer** — 跨平台文本写入，自动处理编码/BOM/CRLF
- **auto-memory** — 对话记忆自动提取，与 MemOS Cloud 配合使用
